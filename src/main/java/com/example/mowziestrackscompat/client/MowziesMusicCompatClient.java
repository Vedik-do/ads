...
public final class MowziesMusicCompatClient {
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (mc.level == null || mc.player == null) {
            ...
            if (duckingMusic || restoringMusic) {
                setMusicVolumeSafe(originalMusicVol > 0 ? originalMusicVol : 1.0f);
                duckingMusic = false;
...
private static float getMusicVolumeSafe() {
    Minecraft mc = Minecraft.getInstance();
    ...
    try {
        ...
        try {
            return (float) mc.options.getSoundSourceVolume(SoundSource.MUSIC);
        } catch (Throwable ignored) {}
    }
    return 1.0f;
}

private static void setMusicVolumeSafe(float vol) {
    Minecraft mc = Minecraft.getInstance();
    if (mc == null || mc.options == null) return;
    float v = Math.max(0.0f, Math.min(1.0f, vol));
    try {
        mc.options.setSoundCategoryVolume(SoundSource.MUSIC, v);
        return;
    } catch (Throwable ignored) {}
    try {
        // Some mappings name this differently
        mc.options.setSoundSourceVolume(SoundSource.MUSIC, v);
